...
export default function ShareProduct({ url, title, className = "", variant = "icon" }) {
  ...
  const popoverContent = (
    <div className="flex w-52 flex-col gap-0.5 py-1">
      {links.map((link) => (
        <a
          key={link.key}
          href={link.href}
          ...
          rel="noopener noreferrer"
          onClick={() => setOpen(false)}
          className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm font-medium text-[#1C1A17] transition-colors duration-150 hover:bg-[#A8823C]/[0.07]"
        >
          <span className={`text-base ${link.color}`}>{link.icon}</span>
          {link.label}
        </a>
      ))}
      <button
        type="button"
        onClick={copyLink}
        className="flex items-center gap-3 rounded-lg px-3 py-2.5 text-left text-sm font-medium text-[#1C1A17] transition-colors duration-150 hover:bg-[#A8823C]/[0.07]"
      >
        <span className="text-base">{copied ? <CheckOutlined className="text-emerald-600" /> : <LinkOutlined className="text-[#A8823C]" />}</span>
        {copied ? "Copied!" : "Copy link"}
      </button>
    </div>
  );
  ...
  if (variant === "button") {
    return (
      <Popover
        ...
        open={canNativeShare ? false : open}
        onOpenChange={setOpen}
        overlayInnerStyle={{ borderRadius: 16, padding: 6 }}
      >
        <button
          type="button"
          onClick={handleClick}
          className={`inline-flex items-center gap-2 rounded-xl border border-[#1C1A17]/15 bg-white px-4 py-2 text-sm font-medium text-[#1C1A17] shadow-sm transition-colors duration-200 hover:border-[#A8823C]/50 hover:text-[#A8823C] ${className}`}
        >
          <ShareAltOutlined aria-hidden="true" />
          Share
        </button>
  ...
  return (
    ...
    >
      <button
        ...
        onClick={handleClick}
        aria-label="Share this product"
        className={`flex h-8 w-8 items-center justify-center rounded-full text-[#1C1A17]/60 transition-all duration-200 hover:scale-110 hover:text-[#A8823C] ${className}`}
      >
        <ShareAltOutlined aria-hidden="true" />
      </button>
    </Popover>
