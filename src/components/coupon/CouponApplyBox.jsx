...
export default function CouponApplyBox({ product, onApplied }) {
  ...
  return (
    <div className="mt-6 overflow-hidden rounded-3xl border border-[#1C1A17]/[0.06] bg-[#FBF7EF] shadow-[0_4px_20px_rgba(28,26,23,0.04)]">
      {/* HEADER */}
      <div className="flex items-start justify-between gap-3 border-b border-[#1C1A17]/[0.06] px-4 py-4 sm:px-5">
        <div className="flex items-start gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl border border-[#D4AF6A]/25 bg-[#A8823C]/[0.08] text-[#A8823C]">
            <TagOutlined />
          </div>
          <div>
            <h3 className="font-semibold text-[#1C1A17]">Have a private offer?</h3>
            <p className="mt-1 text-xs leading-5 text-[#6B6459] sm:text-sm">
              Enter your coupon code to unlock available savings.
            </p>
          </div>
        </div>
        ...
        {isAuthenticated && (
          <Button
            ...
            icon={<HistoryOutlined />}
            onClick={() => setHistoryOpen(true)}
            className="!shrink-0 !text-[#A8823C] hover:!text-[#8A6B2E]"
          >
            History
          </Button>
        )}
      ...
      {!result ? (
        <div className="p-4 sm:p-5">
          <div className="flex flex-col gap-2 sm:flex-row">
            <Input
              ...
              maxLength={40}
              placeholder="Enter coupon code"
              prefix={<TagOutlined className="text-[#A8823C]/60" />}
              className="!rounded-xl !border-[#1C1A17]/10 !bg-white !uppercase !tracking-wide focus-within:!border-[#A8823C]/50 focus-within:!shadow-[0_0_0_3px_rgba(168,130,60,0.1)]"
              onChange={(e) => setCode(e.target.value.toUpperCase())}
              onPressEnter={applyCoupon}
            />
            <Button
             // type="primary"
              ...
              loading={loading}
              onClick={applyCoupon}
              className="!h-11 !rounded-xl !border-0 !bg-[#1C1A17] !font-medium !shadow-none transition-all duration-200 hover:!bg-[#2A2620] !text-[#D4AF6A] "
            >
              Apply
            </Button>
          </div>
          ...
          {!isAuthenticated && (
            <div className="mt-3 flex gap-2 rounded-2xl border border-[#D4AF6A]/25 bg-[#A8823C]/[0.05] p-3 text-xs leading-5 text-[#8A6B2E]">
              <InfoCircleOutlined className="mt-0.5 shrink-0 text-[#A8823C]" />
              <span>Login to apply a coupon and track your savings.</span>
            </div>
          )}

          <div className="mt-4 flex gap-2 rounded-2xl bg-white p-3 text-xs leading-5 text-[#6B6459]">
            <InfoCircleOutlined className="mt-0.5 shrink-0 text-[#A8823C]/70" />
            <span>Coupon eligibility may depend on minimum order value, product selection and offer validity.</span>
          </div>
        </div>
      ) : (
        ...
        <div className="p-4 sm:p-5">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-3">
              <CheckCircleFilled className="mt-1 text-xl text-emerald-600" />
              <div>
                <div className="flex flex-wrap items-center gap-2">
                  <span className="font-semibold text-emerald-700">Coupon Applied</span>
                  <Tag className="!border-emerald-600/25 !bg-emerald-50 !text-emerald-700">
                    {result.coupon?.code || code}
                  </Tag>
                </div>
                <p className="mt-1 text-xs text-[#6B6459]">You save ₹{discount.toLocaleString("en-IN")}</p>
              </div>
            </div>
            <button
              type="button"
              onClick={removeCoupon}
              className="flex h-8 w-8 items-center justify-center rounded-full text-[#8A8377] transition hover:bg-[#9B4444]/10 hover:text-[#9B4444]"
              aria-label="Remove coupon"
            >
              <CloseOutlined />
            </button>
          ...
          <div className="mt-4 rounded-2xl border border-[#1C1A17]/[0.06] bg-white p-4">
            <div className="flex justify-between text-sm">
              <span className="text-[#8A8377]">Product Price</span>
              <span className="font-medium text-[#1C1A17]">₹{price.toLocaleString("en-IN")}</span>
            </div>
            <div className="mt-2 flex justify-between text-sm">
            ...
            <div className="my-3 border-t border-dashed border-[#1C1A17]/10" />
            <div className="flex items-center justify-between">
              <span className="font-semibold text-[#1C1A17]">Estimated Price</span>
              <span className="text-xl font-bold text-[#A8823C]">₹{payableAmount.toLocaleString("en-IN")}</span>
            </div>
          </div>

          <div className="mt-3 rounded-2xl border border-emerald-700/15 bg-emerald-50/60 p-3 text-xs leading-5 text-emerald-800">
            <strong>Coupon applied successfully.</strong> Final order amount may include shipping, taxes or other applicable charges.
          </div>
        </div>
      )}
      ...
      <Modal
        ...
        title={
          <div className="flex items-center gap-2">
            <HistoryOutlined className="text-[#A8823C]" />
            <span>Your Coupon History</span>
          </div>
        }
        footer={null}
      ...
      >
        {historyLoading ? (
          ...
          <div className="max-h-[420px] space-y-3 overflow-y-auto pr-1">
            {history.map((r) => (
              <div key={r.id} className="rounded-2xl border border-[#1C1A17]/[0.06] bg-[#FBF7EF] p-4">
                <div className="flex items-center justify-between gap-2">
                ...
                {r.coupon?.title && (
                  <p className="mt-2 text-sm font-medium text-[#1C1A17]">{r.coupon.title}</p>
                )}
                <div className="mt-2 flex justify-between text-sm">
