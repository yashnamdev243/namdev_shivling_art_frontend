...
export default function AdminSidebar({ mobileOpen = false, onCloseMobile }) {
    ...
    <>
      <div className="flex items-center justify-between px-4 py-6">
        ...
        >
          ...
          <div
            className="flex h-10 w-10 shrink-0 items-center justify-center rounded-full
                 border border-[#D4AF6A]/40 bg-white/[0.04]"
          >
            <span className="text-lg font-semibold leading-none text-[#D4AF6A]">
              ॐ
            </span>
          </div>
          ...
          {(!collapsed || isMobile) && (
            <div className="min-w-0">
              ...
              <p className="truncate text-[11px] leading-tight text-[#D4AF6A]/70">
        ...
        {isMobile ? (
          <button
            onClick={onCloseMobile}
            className="rounded-lg p-2 text-white/40 transition-colors duration-200 hover:bg-white/[0.06] hover:text-white"
            aria-label="Close menu"
          >
            <CloseOutlined />
          </button>
      ...
      <nav className="sidebar-scrollbar flex-1 space-y-1 overflow-y-auto px-3 pb-3">
        {links.map((link) => (
          ...
          >
            {({ isActive }) => (
              <>
                {isActive && (
                  <span className="absolute left-0 top-1/2 h-5 w-[3px] -translate-y-1/2 rounded-full bg-[#D4AF6A]" />
                )}
                <span
                  className={`shrink-0 text-lg ${isActive ? "text-[#D4AF6A]" : ""}`}
                >
                  {link.icon}
                </span>
                {(!collapsed || isMobile) && (
        ...
        <a
          ...
          className={`mt-2 flex items-center gap-3 rounded-xl border-t border-white/[0.06] px-3 py-3 pt-4 text-sm font-medium text-white/45 transition-colors duration-200 hover:text-[#D4AF6A] ${
            collapsed && !isMobile ? "justify-center" : ""
          }`}
        >
          <GlobalOutlined className="shrink-0 text-lg" />
      ...
      <div className="shrink-0 border-t border-white/[0.06] p-3">
        <div
          className={`flex items-center gap-3 rounded-xl px-2 py-2 ${collapsed && !isMobile ? "justify-center" : ""}`}
        >
          <div className="flex h-9 w-9 shrink-0 items-center justify-center rounded-full border border-[#D4AF6A]/30 bg-[#D4AF6A]/[0.08] font-semibold text-[#D4AF6A]">
            {(user?.name || "A")[0].toUpperCase()}
          </div>
          {(!collapsed || isMobile) && (
            <div className="min-w-0">
        ...
        <button
          onClick={() => setLogoutConfirmOpen(true)}
          className={`mt-2 flex w-full items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium text-white/45 transition-colors duration-200 hover:bg-[#9B4444]/[0.12] hover:text-[#E8B4B4] ${
            collapsed && !isMobile ? "justify-center" : ""
          }`}
        >
          <LogoutOutlined />
  ...
  return (
    <>
      {/* DESKTOP SIDEBAR */}
      <aside
        className={`sticky top-0 hidden h-screen shrink-0 flex-col bg-[#151515] text-stone-300 transition-all duration-300 lg:flex ${
          collapsed ? "w-20" : "w-64"
        }`}
      >
        {sidebarContent(false)}
      ...
      {/* MOBILE DRAWER */}
      <aside
        className={`fixed inset-y-0 left-0 z-50 flex h-screen w-72 max-w-[85vw] flex-col bg-[#151515] text-stone-300 shadow-2xl transition-transform duration-300 lg:hidden ${
          mobileOpen ? "translate-x-0" : "-translate-x-full"
        }`}
      >
        {sidebarContent(true)}
