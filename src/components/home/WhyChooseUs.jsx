...
export default function WhyChooseUs() {
  ...
  return (
    <section className="relative overflow-hidden py-6">
      <div className="pointer-events-none absolute left-1/2 top-0 h-72 w-72 -translate-x-1/2 rounded-full bg-[#D4AF6A]/[0.06] blur-[130px]" />
      ...
      <Container>
        ...
        <motion.div
          ...
          viewport={{ once: true, amount: 0.2 }}
          variants={gridVariants}
          className="mt-10 grid gap-5 sm:mt-16 sm:gap-6 md:grid-cols-2 lg:grid-cols-4"
        >
          {t.features.map((item, i) => {
            const Icon = ICONS[i];
            ...
            return (
              <motion.div
                ...
                className="group relative rounded-[22px] border border-[#1C1A17]/[0.08] bg-[#FBF7EF] p-6 shadow-[0_4px_20px_rgba(28,26,23,0.04)] transition-all duration-300 hover:border-[#D4AF6A]/40 hover:shadow-[0_14px_36px_rgba(28,26,23,0.08)] sm:p-7"
              >
                {/* Icon */}
                <div className="flex h-12 w-12 items-center justify-center rounded-2xl border border-[#D4AF6A]/25 bg-[#A8823C]/[0.08] text-xl text-[#A8823C] transition-transform duration-300 group-hover:scale-110">
                  <Icon aria-hidden="true" />
                </div>

                {/* Title */}
                <h3 className="mt-5 text-lg font-semibold text-[#1C1A17] sm:text-xl">
                  {item.title}
                </h3>

                {/* Divider */}
                <div className="mt-2.5 h-px w-8 bg-[#D4AF6A]/40 transition-all duration-300 group-hover:w-14" />

                {/* Description */}
                <p className="mt-4 text-sm leading-6 text-[#6B6459] sm:text-[15px] sm:leading-7">
                  {item.description}
                </p>

                {/* Badge */}
                <span className="mt-5 inline-block rounded-full bg-[#1C1A17]/[0.04] px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-[#A8823C]">
                  {t.badge}
                </span>
              </motion.div>
            );
