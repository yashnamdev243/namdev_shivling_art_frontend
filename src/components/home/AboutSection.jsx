    ...
    <section className="relative overflow-hidden py-6">
      <Container>
        <div className="grid items-center gap-12 lg:grid-cols-2 lg:gap-20">
          ...
          >
            {/* Subtle offset frame */}
            <div className="pointer-events-none absolute -bottom-3 -right-3 h-full w-full rounded-[24px] border border-[#D4AF6A]/25 sm:rounded-[32px]" />

            {/* Image */}
            <div className="relative overflow-hidden rounded-[24px] border border-[#1C1A17]/[0.06] bg-white p-2 shadow-[0_25px_70px_rgba(28,26,23,0.14)] sm:rounded-[32px] sm:p-3">
              {isLoading ? (
                <Skeleton.Image active className="!h-[320px] !w-full sm:!h-[450px] lg:!h-[620px]" />
              ) : (
                <img
                  ...
                  loading="lazy"
                  alt="Namdev Narmadeshwar Shivling Art - Authentic Narmadeshwar Shivling Manufacturer in Bakawan Khargone Madhya Pradesh"
                  className="h-[320px] w-full rounded-[18px] object-cover transition duration-700 hover:scale-[1.03] sm:h-[450px] sm:rounded-[24px] lg:h-[620px]"
                />
              )}
            </div>
            ...
            <motion.div
              whileHover={{ y: -4 }}
              className="absolute -bottom-5 left-4 sm:-bottom-6 sm:left-8"
            >
              <Button
                size="large"
                onClick={() => navigate("/about")}
                className="!flex !h-12 !items-center !gap-2 !rounded-2xl !border-none !bg-[#1C1A17] !px-6 !text-sm !font-medium !text-[#F2E3C8] !shadow-[0_14px_34px_rgba(28,26,23,0.25)] transition-all duration-200 hover:!-translate-y-0.5 hover:!bg-[#2A2620] sm:!h-14 sm:!px-8 sm:!text-base"
              >
                {t.cta}
                <ArrowRightOutlined className="text-[#D4AF6A]" />
              </Button>
            </motion.div>
          </motion.div>
          ...
          <motion.div
            ...
            className="mt-10 lg:mt-0"
          >
            <span className="inline-flex items-center gap-2 rounded-full border border-[#D4AF6A]/25 bg-[#A8823C]/[0.06] px-4 py-1.5 text-[11px] font-semibold uppercase tracking-[0.2em] text-[#A8823C] sm:px-5 sm:tracking-[0.25em]">
              {t.badge}
            </span>

            <h2 className="mt-5 text-3xl font-bold leading-tight tracking-tight text-[#1C1A17] sm:mt-6 sm:text-4xl lg:text-5xl">
              {t.titleLine1}
              <span className="block text-[#A8823C]">{t.titleLine2}</span>
            </h2>

            <p className="mt-6 text-base leading-8 text-[#6B6459] sm:mt-8 sm:text-lg sm:leading-9">
                {t.paragraph1}
            </p>

            <p className="mt-5 text-base leading-8 text-[#6B6459] sm:mt-6 sm:text-lg sm:leading-9">
             {t.paragraph1}
            </p>

            <div className="mt-10 grid grid-cols-2 gap-6 sm:mt-12 sm:gap-10">
              {t.stats.map(([value, label], i) => (
                <motion.div
                  key={label}
                  ...
                  transition={{ duration: 0.4, delay: i * 0.08 }}
                >
                  <h3 className="text-2xl font-bold text-[#A8823C] sm:text-4xl">{value}</h3>
                  <span className="mt-2 mb-2 block h-px w-8 bg-[#D4AF6A]/40" aria-hidden="true" />
                  <p className="text-sm text-[#6B6459] sm:text-base">{label}</p>
                </motion.div>
              ))}
            </div>
          </motion.div>
