...
import { AppstoreOutlined, PlusOutlined, PercentageOutlined } from "@ant-design/icons";
...
export default function Dashboard() {
  ...
  const { data: productsData, isLoading: loadingProducts } = useProducts({ limit: 5 });
    ...
    <>
      ...
      >
        <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-center">
          ...
          <Avatar size={64} className="hidden shrink-0 !bg-white text-orange-600 sm:flex" icon={<AppstoreOutlined />} />
        </div>
      </motion.div>
      ...
      <div className="grid gap-4 sm:gap-6 md:grid-cols-2 xl:grid-cols-3">
        {statCards(stats).map((item, i) => (
          <motion.div key={item.title} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}>
            <Link to={item.link}>
              <StatCard
                label={item.title}
                value={item.value}
                icon={
                  <div className={`rounded-xl bg-gradient-to-r ${item.color} p-3 text-white`}>{item.icon}</div>
                }
                highlight={item.warn}
              />
            </Link>
      ...
      <div className="mt-4 grid gap-4 sm:mt-6 sm:gap-6 md:grid-cols-2 xl:grid-cols-4">
        ...
        <EngagementCard
          ...
          value={redemptionLoading ? "…" : redemptionData?.redemptions?.length || 0}
      ...
      <div className="mt-6 grid gap-6 sm:mt-8 xl:grid-cols-3">
        ...
        <Card
          ...
          title="Recently Added Products"
          extra={
            <Link to={ROUTES.adminProducts} className="flex items-center gap-1 text-sm font-medium text-orange-600">
          ...
          {loadingProducts ? (
            ...
            <div className="space-y-3">
              {products.map((item) => (
                ...
                >
                  <img
                    src={item.image ? `${FILE_BASE_URL}/uploads/${item.image}` : "https://placehold.co/80"}
                  ...
                  <div className="shrink-0 text-right">
                    <div className="font-bold text-amber-600">{formatCurrency(item.price)}</div>
                    <span
        ...
        <div className="space-y-6">
          <Card title="Inventory Health" className="!rounded-3xl">
            <Progress percent={stockHealthPercent} strokeColor={{ "0%": "#f97316", "100%": "#16a34a" }} />
          ...
          <Card title="Quick Actions" className="!rounded-3xl">
            <div className="flex flex-col gap-3">
              <Link to={ROUTES.adminProducts}>
                <Button icon={<FiPlus />} block size="large" type="primary" className="!rounded-xl">
                  Add Product
                </Button>
              </Link>
              <Link to="/admin/reviews">
                <Button icon={<FiMessageSquare />} block size="large" className="!rounded-xl">
                  Moderate Reviews {pendingReviews.length > 0 && `(${pendingReviews.length})`}
                </Button>
              </Link>
              <Link to="/admin/coupons">
                <Button icon={<PercentageOutlined />} block size="large" className="!rounded-xl">
                  Manage Coupons
                </Button>
              </Link>
            </div>
      ...
      <div className="mt-6 grid gap-6 sm:mt-8 xl:grid-cols-2">
        <Card
          ...
          extra={
            <Link to="/admin/activity" className="flex items-center gap-1 text-sm font-medium text-orange-600">
              View All <FiArrowRight />
            </Link>
          }
        >
          {activityLoading ? (
            ...
            <div className="space-y-3">
              {activities.map((item) => {
                ...
                return (
                  <div key={item.id} className="flex items-center justify-between gap-3 rounded-xl border border-orange-50 px-3 py-2.5">
                    <div className="min-w-0">
                      <p className="truncate text-sm font-semibold text-slate-900">{item.user?.name || "Unknown"}</p>
                      <p className="truncate text-xs text-gray-500">{item.product?.name || item.user?.email || ""}</p>
                    </div>
        ...
        <Card
          ...
          className="!rounded-3xl"
          extra={
            <Link to="/admin/coupons" className="flex items-center gap-1 text-sm font-medium text-orange-600">
              View All <FiArrowRight />
            </Link>
          }
        >
          {redemptionLoading ? (
            ...
            <div className="space-y-3">
              {recentRedemptions.map((r) => (
                <div key={r.id} className="flex items-center justify-between gap-3 rounded-xl border border-orange-50 px-3 py-2.5">
                  <div className="min-w-0">
                    <p className="truncate text-sm font-semibold text-slate-900">{r.user?.name || "Customer"}</p>
                    <p className="truncate text-xs text-gray-500">{r.coupon?.code || "—"}</p>
                  </div>
                  <div className="shrink-0 text-right">
...
function EngagementCard({ to, label, value, icon, color, badge }) {
  return (
    <Link to={to}>
      ...
      >
        <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br ${color} text-white sm:h-12 sm:w-12`}>
          {icon}
        </div>
        <div className="min-w-0">
          ...
          {badge && <span className="mt-0.5 inline-block text-[11px] font-semibold text-orange-600">{badge}</span>}
        </div>
      </motion.div>
    </Link>
  );
