      ...
      <section className="relative overflow-hidden bg-gradient-to-b from-[#FBF7EF] via-white to-white py-12 sm:py-16 md:py-24 lg:py-28 px-4 sm:px-6 lg:px-8">
        ...
        <div className="container mx-auto max-w-7xl px-0 sm:px-5">
          <div className="mb-12 text-center sm:mb-14">
            ...
            <span className="mx-auto mt-6 block h-px w-16 bg-[#D4AF6A]/40" aria-hidden="true" />
          </div>
            ...
            <>
              <Image.PreviewGroup>
                <motion.div
                  ...
                  className="grid grid-cols-2 gap-2 sm:grid-cols-2 sm:gap-2 lg:grid-cols-3 xl:grid-cols-4"
                >
                  {pageImages.map((src, index) => {
                    const globalIndex = (page - 1) * PAGE_SIZE + index;
                    ...
                    return (
                      <motion.div
                        ...
                        whileHover={{ y: -6 }}
                        className="group overflow-hidden rounded-[4px] border border-[#1C1A17]/[0.06] bg-white p-1.5 shadow-[0_4px_20px_rgba(28,26,23,0.05)] transition-shadow duration-300 hover:shadow-[0_20px_50px_rgba(28,26,23,0.12)] sm:rounded-[8px] sm:p-2"
                      >
                        <div className="relative h-40 overflow-hidden rounded-sm bg-[#F7F2E7] sm:h-64 sm:rounded-md lg:h-80">
                          <Image
                            src={src}
                            alt={`${t.imageAlt} ${globalIndex + 1}`}
                            wrapperClassName="!block !h-full !w-full"
                            ...
                            preview={{
                              mask: (
                                <span className="flex items-center gap-2 text-xs font-medium text-[#F2E3C8] sm:text-sm">
                                  <EyeOutlined aria-hidden="true" /> {t.view}
                                </span>
                              ),
                            }}
                          ...
                          <div className="pointer-events-none absolute left-2 top-2 flex h-7 w-7 items-center justify-center rounded-full border border-white/20 bg-[#1C1A17]/50 text-[10px] font-semibold text-[#F2E3C8] backdrop-blur-md sm:left-4 sm:top-4 sm:h-9 sm:w-9 sm:text-xs">
                            {String(globalIndex + 1).padStart(2, "0")}
                          </div>
                        </div>
                      </motion.div>
              ...
              {totalPages > 1 && (
                <div className="mt-10 flex justify-center sm:mt-14 [&_.ant-pagination-item-active]:!border-[#1C1A17] [&_.ant-pagination-item-active]:!bg-[#1C1A17] [&_.ant-pagination-item-active_a]:!text-[#F2E3C8] [&_.ant-pagination-item:hover]:!border-[#A8823C]/50">
                  <Pagination
                    current={page}
                    pageSize={PAGE_SIZE}
                    total={images.length}
